...
function GaussSeidel(Toldf, awf, apf, aef, bf)
    Tnew = zero(Toldf)
    Tnew[1] = bf[1] / apf[1]
    Tnew[2] = (awf[2]*Tnew[1] + aef[2]*Toldf[3] + bf[2]) / (apf[2])
    for i in 3:length(Toldf)-2
        Tnew[i] = (awf[i]*Tnew[i-1] + aef[i]*Toldf[i+1] + bf[i]) / (apf[i])
    end
    Tnew[end-1] = (awf[end-1]*Tnew[end-2] + aef[end-1]*Toldf[end] + bf[end-1]) / (apf[end-1])
    Tnew[end] = (awf[end]*Tnew[end-1] + aef[end]*Toldf[end] + bf[end]) / (apf[end])
    return Tnew
end

function GaussSeidelRelaxed(Toldf, awf, apf, aef, bf,alphaf)
    Tnew = zero(Toldf)
    Tnew[1] = bf[1] / apf[1]
    Tnew[2] = Toldf[2] + ((awf[2]*Tnew[1] + aef[2]*Toldf[3] + bf[2]) / (apf[2]) - Toldf[2]) * alphaf
    for i in 3:length(Toldf)-2
        Tnew[i] = Toldf[i] + ((awf[i]*Tnew[i-1] + aef[i]*Toldf[i+1] + bf[i]) / (apf[i]) - Toldf[i]) * alphaf
    end
    Tnew[end-1] = Toldf[end-1]+((awf[end-1]*Tnew[end-2] + aef[end-1]*Toldf[end] + bf[end-1]) / (apf[end-1]) - Toldf[end-1]) * alphaf
    Tnew[end] = Toldf[end] + ((awf[end]*Tnew[end-1] + aef[end]*Toldf[end] + bf[end]) / (apf[end]) - Toldf[end]) * alphaf
    return Tnew
end
